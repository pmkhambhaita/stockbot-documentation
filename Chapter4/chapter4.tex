...
\begin{table}[htbp]
	...
	\begin{tabular}{|l|c|}
		...
		Tests failed/Partially Met & Z \\
		\hline
		Fixed issues (identified \& addressed this sprint) & 1 \\ % (A* validation fix)
		\hline
	\end{tabular}
...
\begin{enumerate}
	...
	\end{itemize}
\end{enumerate}
Due to time constraints, this implementation is basic. Obstacles are permanent once added for a session (until 'Clear All') and cannot be removed in bulk. They are simply marked as non-traversable.
...
\subsubsection{Prototype details:}
User can now enter an \verb|itemID| and click "Add Obstacle". If valid, the internal grid state is updated, but this has no effect on pathfinding or visualisation yet.
