...
\begin{table}[htbp]
	...
	\begin{tabularx}{\textwidth}{|l|X|p{4.5cm}|p{1.5cm}|c|}
		...
		\hline
		T5.1.3 & Click Clear All & Visualisation canvas clears/resets. & Met & X \\
		\hline
	\end{tabularx}
...
\subsubsection{Technical Approach}
My approach involved implementing A* first, integrating it into the GUI, and then setting up the GA framework:
...
\subsubsection{Iteration 2: Genetic Algorithm Framework \& A* Fix}
\begin{itemize}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item A* Validation Fix: Modified the neighbour check inside \verb|a_star_search|. Instead of manually checking bounds \verb|0 <= nx < self.grid.rows...|, it now calls the unified validation function: \verb|valid, _ = spa.validate_point(nx, ny, self.grid.rows, self.grid.cols, allow_start_end=True)|. The condition became \verb|if valid and (nx, ny) not in came_from:|.
