...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.3\linewidth]{Flowcharts/dbquery.png}
\end{figure}
...
% Sprint Berners-Lee Tests (T2.F.X)
...
% Sprint Dijkstra Final Tests (T4.F.X) - None listed, skipped in renumbering

% Sprint Engelbart Final Tests (Renumbered T4.F.X, originally T5.F.X)
...
% Sprint Floyd Final Tests (Renumbered T5.F.X, originally T6.F.X)
...
\hline
T5.F.2 & Run same path, A* selected, Optimisation OFF. Note length. & Shortest path segments found using A*. Points visited in order entered. Total length matches T1.F.1 length. & Verifies A* path calculation and compares length to BFS (should be equal). \\ % Updated test ID ref
\hline
T5.F.3 & Run same path, BFS selected, Optimisation ON. Note length. & Path segments via BFS. Point order potentially changed by GA for shorter total length than T1.F.1 length. & Tests the combined effect of GA optimisation with BFS path segments. \\ % Updated test ID ref
\hline
T5.F.4 & Run same path, A* selected, Optimisation ON. Note length. & Path segments via A*. Point order potentially changed by GA. Total length matches T3.F.3 length (should be same shortest path). & Tests the combined effect of GA optimisation with A* path segments. \\ % Updated test ID ref
\hline

% Sprint Gates Final Tests (Renumbered T6.F.X, originally T7.F.X)
